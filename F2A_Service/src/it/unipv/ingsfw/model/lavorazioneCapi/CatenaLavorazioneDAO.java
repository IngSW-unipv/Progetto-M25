...
public class CatenaLavorazioneDAO implements ICatenaLavorazioneDAO {
	...
	public boolean insertCatena(CatenaLavorazione c) {
		...
		try {
			...
			st1.setInt(2, Integer.parseInt(c.getTipoLavaggio().toString()));
	...
	public CatenaLavorazione selectCatenaByStazione(ObservableStazioneLavoro input) {
		...
		String idCatena = input.getIdCatena();

		conn = DBConnection.startConnection(conn);
		PreparedStatement st1;
		ResultSet rs1;
		CatenaLavorazione c = null;
		...
		try {
			...
			String query = "SELECT * FROM CATENELAVORAZIONE WHERE IDCATENA ='" + input.getIdCatena() + "'";
			st1 = conn.prepareStatement(query);
			...
			while (rs1.next()) {
				...
				c = new CatenaLavorazione(rs1.getString(1), TipoLavaggio.valueOf(listaTipiLavaggio.get(rs1.getInt(2))));
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * for(CatenaLavorazione c : listaCatene) System.out.println(c);
		 * 
		 * CatenaLavorazione cl1 = new CatenaLavorazione("CAT001", null);
		 * ArrayList<ObservableStazioneLavoro> listaStazioni =
		 * cl.selectStazioniByCatena(cl1);
		 * 
		 * for(ObservableStazioneLavoro o : listaStazioni) System.out.println(o);
		 * 
		 * CatenaLavorazione cl2 = new CatenaLavorazione("CAT002", TipoLavaggio.PELLE);
		 * boolean t = cl.insertCatena(cl2); if(t) System.out.println(cl2);
		 */

		/*
		 * CatenaLavorazione cl2 = new CatenaLavorazione("CAT002", TipoLavaggio.PELLE);
		 * //System.out.println(cl.checkCatenaAlreadyExists(cl2)); boolean t =
		 * cl.insertCatena(cl2); if(t) System.out.println(cl2);
		 * 
		 * CatenaLavorazione cl3 = new CatenaLavorazione("CAT003", TipoLavaggio.COTONE);
		 * //System.out.println(cl.checkCatenaAlreadyExists(cl2)); boolean t1 =
		 * cl.insertCatena(cl3); if(t1) System.out.println(cl3);
		 */
